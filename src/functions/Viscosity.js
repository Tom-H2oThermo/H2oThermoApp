...
export function Visc(temperature, volume, pressure) {
  ...
  } else throw Errors.PressureOrTempNotInRange;
}

export function ViscR3(temperature, volume, dVdP_T, pressure) {
  // Covers 0 < pressure<= 300 MPa and Temperature < 1173.15K and (Temp > Tmelt (if greater than p triple) or Temp > 273.16 (if less than p triple))
  ...
  if (
    ...
    if (temperature > 645.91 && temperature < 650.77 && rho > 245.8 && rho < 405.3) {
      ...
      var volume_T_crit = Region3Calculations.Volume_fPT(pressure, Constants.Tc_H2O); // use pressure and T_crit to calculate volume_T_crit
      dVdP_T = 1 / Region3Calculations.Properties_fVT(volume, temperature)[16]; // See if this changes from the input dVdP_T
      ...
      volume = Volume; // I think that that lines 58 and 59 were changing the volume, pressure and temperature so I added lines 55-57 & 60 - 62.  Testing now
      pressure = Pressure;
      temperature = Temperature;
      var zeta = (Constants.pc_H2O / Constants.RHOc_H2O) * -(dVdP_T / Math.pow(volume, 2)); // R12-08 Eq21a. The differential reciprocol rule is needed to convert from rho to v, hence the (1/v)^2 term
      var zetaTr = (Constants.pc_H2O / Constants.RHOc_H2O) * -(dVdP_T_crit / Math.pow(volume_T_crit, 2)); // / R12-08 Eq21a.  Properties[52] is dv/dp_T
      var delta_chi_dim = rho_dim * (zeta - zetaTr * (Constants.T_R / T_dim)); // R12-08 Eq. 21
      if (delta_chi_dim < 0) {
        delta_chi_dim = 0;
      }
      var xi = Constants.xi_0 * Math.pow(delta_chi_dim / Constants.Gamma_0, Constants.ni / Constants.gamma); // R12-08 Eq. 20
      var Y = 0;
...
export function ViscTest(temperature, volume) {
  // used for testing R12 tables 4&5 values.  Doesn't check for valid pressures (use with caution).
  // H2oProperties Viscosity_fTV(temperature, volume) calls this function only if not(temperature > 645.91 && temperature < 650.77 && rho > 245.8 && rho < 405.3)
  var T_dim = temperature / T_ref;
  var rho_dim = 1 / (volume * rho_ref);
  var mu_0 = 0;
  var mu_2 = 1;
